...
# 4. Initialize Memory & Strict Domain System Prompt (Clean text only - No emojis here to prevent ASCII errors)
if "messages" not in st.session_state:
    st.session_state.messages = [
        {
            "role": "system", 
            "content": (
                "Identity and Domain Restriction:\n"
                ...
                "(such as medical diagnoses, mental health, coding, cars, pop culture, history, general technology), "
                "you must absolutely refuse to answer. Use this exact refusal statement:\n"
                "'I am a dedicated Fitness and Nutrition AI Assistant. I am strictly restricted to answering questions regarding workouts, diet, and fitness tracking, so I cannot assist with that topic. Please let me know how I can help you reach your physical fitness goals.'\n\n"
                "Core Feature Requirements:\n"
                "1. Food Logging: Analyze natural meal descriptions (such as aloo paratha with yogurt). Instantly calculate and return an estimated breakdown of Calories, Protein, Carbs, and Fats.\n"
                "2. Calorie Deficit Tracking: Calculate user TDEE when provided age, weight, height, and activity level. Establish a daily target for fat loss or muscle gain, tracking eaten vs burned metrics with daily summaries detailing calories remaining and protein targets.\n"
...
# 6. Generate Response using the conversation context transcript
    with st.chat_message("assistant"):
        try:
            # We explicitly clean/encode the context string into ascii-safe characters just in case the user types an emoji
            chat_context = "\n".join([f"{m['role'].capitalize()}: {m['content']}" for m in st.session_state.messages])
            clean_context = chat_context.encode('ascii', 'ignore').decode('ascii')
